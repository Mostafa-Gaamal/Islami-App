...
import com.example.islami.Quran_adadpter
import com.example.islami.Quran_inside
...
class QuranFragment : Fragment() {
    ...
    lateinit var viewIt : FragmentQuranBinding
   private lateinit var adapterQuran:Quran_adadpter
    ...
    override fun onCreateView(
        ...
        viewIt = FragmentQuranBinding.inflate(layoutInflater)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        adapterQuran = Quran_adadpter(list, list2)

        val recyclerView = view.findViewById<RecyclerView>(R.id
            .rv_quran)
        recyclerView.adapter = adapterQuran
        adapterQuran.setonClickObject=Quran_adadpter.setClick{ itemView, position ->
            val intent = Intent(view.context, Quran_inside::class.java)
            val b = position+1
            val inputStream = context?.assets?.open("$b.txt")
            val fileContent = inputStream?.bufferedReader().use { it?.readText() }?.trim()
            intent.putExtra("name" , fileContent)
